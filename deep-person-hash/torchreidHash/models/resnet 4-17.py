...
class ResNet(nn.Module):
    ...
    def __init__(
        ...
        **kwargs
        ...
        feats=256
        if(self.hashbit<=128):
            self.fc_encode_mgn = nn.Linear(12288, self.hashbit)
        else:
            self.fc_encode_mgn = nn.Linear(12288, self.hashbit-self.hashbit//2)
        self.parts = 6
        ...
        self.feature_dim_pcb = reduced_dim
        self.classifier_pcb = nn.ModuleList(
            [
                nn.Linear(self.feature_dim_pcb, num_classes)
    ...
    def forward(self, x):
        ...
        f_pcb = self.featuremaps_pcb(x)
        
        #print(f_pcb.shape)
        v_g = self.parts_avgpool(f_pcb)
        ...
        for i in range(self.parts):
            ...
            y_i = self.classifier_pcb[i](v_h_i)
            y_pcb.append(y_i)
            y=torch.cat([y,y_i],dim=1)
        v_g = F.normalize(v_g, p=2, dim=1)
        v_g=v_g.view(v_g.size(0), -1)
        h_pcb=self.fc_encode_mgn(v_g)
