...
class ResNet(nn.Module):
    ...
    def __init__(
        ...
        **kwargs
        ...
        feats=256
        if(self.hashbit<=128):
            self.fc_encode_mgn = nn.Linear(feats*4, self.hashbit)
        else:
            self.fc_encode_mgn = nn.Linear(feats*4, self.hashbit-self.hashbit//2)
        ...
        # self.atten1=HarmAttn(2048)
        # self.atten2=RegionAttn()
        # self.atten3=RegionAttn()
    ...
    def forward(self, x):
        ...
        # print("zp3 ori:",zp3.shape)
        # atten=self.atten1(zp3)
        # zp3=zp3*atten
        # print("zp3 ori2:",zp3.shape)
        z0_p3 = zp3[:, :, 0:1, :]
        z1_p3 = zp3[:, :, 1:2, :]
        z2_p3 = zp3[:, :, 2:3, :]
        # print("zp3:",zp3.shape)
        ...
        mgn_part=torch.cat([fg_p3,f0_p3,f1_p3,f2_p3,l_p3,l0_p3,l1_p3,l2_p3],dim=1)
        feats_mgn=torch.cat([fg_p3,f0_p3,f1_p3,f2_p3],dim=1)
        h_mgn=self.fc_encode_mgn(feats_mgn)
        b_mgn=hash_layer(h_mgn)
        
        if(self.hashbit<=64):
            b_return=b
            h_return=h
        else:
            h_return=torch.cat([h,h_mgn],dim=1)
            b_return=torch.cat([b,b_mgn],dim=1)
        
        # test=feats_mgn[:4]
        # print("global:",fg_p3.shape)
        # print(v.shape)
        # print(test.shape)
        v=torch.cat([v,fg_p3,f0_p3,f1_p3,f2_p3],dim=1)
        y=torch.cat([y,l_p3,l0_p3,l1_p3,l2_p3],dim=1)
        if self.loss == 'softmax':
            return y
        elif self.loss == 'triplet':
            return y, v,h_return,b_return#,fg_p3#,b_weighted#,y_pcb,v_pcb_return
        else:
