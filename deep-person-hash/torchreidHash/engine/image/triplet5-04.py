...
class ImageTripletEngine(Engine):
    ...
    def train(
        ...
        for batch_idx, data in enumerate(self.train_loader):
            ...
            loss_x = self._compute_loss(self.criterion_x, outputs, pids)+self._compute_loss(self.criterion_x, b_classify, pids)
            ...
            var=torch.zeros(1)
            var=var.cuda()
            metric_loss = torch.max(loss12-loss1.detach(),Variable(var)) + torch.max(loss12-loss2.detach(),Variable(var)) + torch.max(loss23-loss2.detach(),Variable(var)) + torch.max(loss23-loss3.detach(),Variable(var)) + torch.max(loss34-loss3.detach(),Variable(var)) + torch.max(loss34-loss4.detach(),Variable(var)) +torch.max(loss45-loss4.detach(),Variable(var)) + torch.max(loss45-loss5.detach(),Variable(var)) +torch.max(loss56-loss5.detach(),Variable(var)) + torch.max(loss56-loss6.detach(),Variable(var))
